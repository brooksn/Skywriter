...
exports.discoverRels = function(params, callback) {
	...
	Alloy.Globals.tent.sendRequest({
		...
		for (var ent in subs) {
			var combo = Alloy.Globals.skywriter.uenc(ent, params.account);
...
exports.fetchCursor = function(account) {
	Alloy.Globals.tent.sendRequest({
		...
		if (cursorcallback.success && cursorcallback.body.posts[0]) {
			Alloy.Globals.cursor[cursorcallback.body.posts[0].entity] = cursorcallback.body.posts[0];
			alert('good: ' + JSON.stringify(Alloy.Globals.cursor));
		}
	});
};
